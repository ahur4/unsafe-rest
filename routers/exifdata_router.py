...
@app.post('/delete-exif-img', tags=["Exif Data"], response_class=JSONResponse)
...
@app.post('/edit-exif-img', tags=["Exif Data"], response_class=FileResponse)
async def edit_exif_img_router(file: bytes = File(...), key: str = Form(...), value: str = Form(...)):
    ...
    with open(file_path, "wb") as f:
        f.write(file)
        f.close()
    isEdited = unsafe.edit_exif_image(path=file_path, key=key, value=value)
    return file_path


@app.post('/extract-exif-img', tags=["Exif Data"], response_class=JSONResponse)
